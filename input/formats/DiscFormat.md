...
This document describes the YAML format for defining one or more discs of particles.

To define a disc, you create a top-level block with the `format` key set to `"Disc"`.

## Disc Block

The disc block has the following keys:

- `format`: A string describing the format, in this case `"Disc"`.
- `discs`: A list of disc objects.

## disc Object

Each object in the `discs` list defines a single disc and has the following keys:

#!/usr/bin/env python3
"""
Plot a temperature diagram T(t) from a CSV file with rows:
...
python ./plot_temperature.py ./<FILE>.csv --delta-t <DELTA_T> --start-time <START_TIME> --out <FILE>.png
"""
...
def read_iteration_temperature_csv(path: Path) -> Tuple[List[float], List[float]]:
    """
    ...
    if not iterations:
        raise ValueError(
            f"No numeric data found in '{path}'. Expected rows like: <iteration>,<temperature>"
        )

    return iterations, temps


def main() -> None:
    p = argparse.ArgumentParser(description="Plot temperature diagram T(t) from CSV.")
    p.add_argument("csv", type=Path, help="Input CSV file (<iteration>,<temperature>)")
    ...
    p.add_argument(
        "--title",
        default="Temperature T(t)",
        help="Plot title (default: Temperature T(t))",
    )
    p.add_argument(
        "--out",
        type=Path,
    ...
    iterations, temps = read_iteration_temperature_csv(args.csv)

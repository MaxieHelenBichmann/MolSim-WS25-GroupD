...
class PeriodicTest : public testing::Test {
   protected:
    ...
    Periodic left_boundary2_d{BoundaryLocation::LEFT, dimension2_d, cutoff, 2};
    Periodic right_boundary2_d{BoundaryLocation::RIGHT, dimension2_d, cutoff, 2};
    Periodic front_boundary2_d{BoundaryLocation::FRONT, dimension2_d, cutoff, 2};
    Periodic back_boundary2_d{BoundaryLocation::BACK, dimension2_d, cutoff, 2};
...
TEST_F(PeriodicTest, Particle2DTeleport3) {
    R3 x = {-1.0, 11.0, .0};
    Particle p = Particle(x, zero, 1.0, 1.0, 1.0, 0);
    R3 expected_position = {9.0, 1.0, .0};
    Particle p1(p);
    Particle p2(p);
    Particle p3(p);
    p1.getX() = {-1.0, 11.0, .0};
    ...
    EXPECT_TRUE(p.getX() == expected_position);
    EXPECT_TRUE(std::ranges::find(p.getMirrorPositions(), p1.getX()) != p.getMirrorPositions().end());
    EXPECT_TRUE(std::ranges::find(p.getMirrorPositions(), p2.getX()) != p.getMirrorPositions().end());

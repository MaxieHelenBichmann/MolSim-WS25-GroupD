...
TEST(VectorTest, TestConstructor) {
    ...
    for (size_t idx = 0; idx < 10; idx++) EXPECT_DOUBLE_EQ(R10[idx], 0.);
    for (size_t idx = 0; idx < 4; idx++) EXPECT_EQ(Z4[idx], 0);
...
TEST(VectorTest, TestMove) {
    ...
    // move constructor
    Vector<double, 3> moved(std::move(R3));
...
TEST(VectorTest, TestPointwiseAddition) {
    Vector<double, 3> aR3(13.5, 69., .01);
    Vector<double, 3> bR3(6.1, -2., 50.);

    Vector<int, 2> aZ2(22, 61);
    Vector<int, 2> bZ2(74, 9);

    EXPECT_DOUBLE_EQ((aR3 + bR3)[0], 19.6);
    EXPECT_DOUBLE_EQ((aR3 + bR3)[1], 67.);
    EXPECT_DOUBLE_EQ((aR3 + bR3)[2], 50.01);

    EXPECT_EQ((aZ2 + bZ2)[0], 96);
    EXPECT_EQ((aZ2 + bZ2)[1], 70);

    EXPECT_TRUE((aZ2 + bZ2) == (bZ2 + aZ2));
}
...
TEST(VectorTest, TestPointwiseSubtraction) {
    ...
    EXPECT_DOUBLE_EQ((aR3 - bR3)[0], 7.4);
    EXPECT_DOUBLE_EQ((aR3 - bR3)[1], 71.);
    EXPECT_DOUBLE_EQ((aR3 - bR3)[2], -49.99);

    EXPECT_EQ((aZ2 - bZ2)[0], -52);
    EXPECT_EQ((aZ2 - bZ2)[1], 52);
}
...
TEST(VectorTest, TestPointwiseMultiplication) {
    ...
    EXPECT_DOUBLE_EQ((aR3 * bR3)[0], 82.35);
    EXPECT_DOUBLE_EQ((aR3 * bR3)[1], -138.);
    EXPECT_DOUBLE_EQ((aR3 * bR3)[2], 0.5);

    EXPECT_EQ((aZ2 * bZ2)[0], 1628);
    EXPECT_EQ((aZ2 * bZ2)[1], 549);
...
TEST(VectorTest, TestPointwiseDivision) {
    Vector<double, 3> aR3(13.5, 69., .01);
    Vector<double, 3> bR3(0.5, -2., 50.);

    Vector<int, 2> aZ2(22, 61);
    Vector<int, 2> bZ2(74, 9);

    EXPECT_DOUBLE_EQ((aR3 / bR3)[0], 27.);
    EXPECT_DOUBLE_EQ((aR3 / bR3)[1], -34.5);
    EXPECT_DOUBLE_EQ((aR3 / bR3)[2], 0.0002);

    EXPECT_EQ((aZ2 / bZ2)[0], 0);
    EXPECT_EQ((aZ2 / bZ2)[1], 6);
}
...
TEST(VectorTest, TestInnerProduct) {
    Vector<int, 3> aZ3(1, 2, 3);
    Vector<int, 3> bZ3(4, -1, 0);
    Vector<double, 2> aR2(4.5, 1.125);
    Vector<double, 2> bR2(0.01, 56.7);

    EXPECT_EQ((Vector<int, 3>::scalarProduct(aZ3, bZ3)), 2);
    EXPECT_EQ((Vector<double, 2>::scalarProduct(aR2, bR2)), 63.8325);
}
...
TEST(VectorTest, TestExplicitDeref) {
    ...
    // const
    const Vector<int, 3> cZ3(69, 1213);
    const std::array<int, 3>& carr = *cZ3;
    EXPECT_EQ(carr[0], 69);

...
std::vector<Particle> Periodic::mirrorParticle(Particle& p) {
    ...
    addMirrorParticle(p.getX() + shift, MIRROR_IDX_LOOKUP[(is2D ? 58 : 48) + location_idx], p, mirrored_particles);

    // 2 handle edges and corners
    size_t idx = getIdx(p);
    if (idx <= 3U) {  // if p in some 3D edge = 2D corner
        uint8_t mirror_idx = MIRROR_IDX_LOOKUP[idx + (is2D ? 54 : 8 * location_idx)];
...
size_t Periodic::getIdx(Particle& p) {
    ...
    if (!is2D) {  // handle 3D corner and edge indicies
        ...
        if (axis1_small && axis2_small) {
            return 4;
        }
        if (axis1_small && axis2_big) {
            return 5;
        }
        if (axis1_big && axis2_small) {
            return 6;
        }
        if (axis1_big && axis2_big) {
            return 7;
        }
        if (axis1_small) {
            return 0;

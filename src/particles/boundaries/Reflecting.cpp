...
void Reflecting::applyBoundary(Particle& p, const PairwiseForceSource& force) noexcept {
    /**
     ...
     * This means that if we want to be sure that the Lennard-Jones-Force actually pushes the particle away from the
     * ghost instead of having the ghost attract it, we need to make sure that the distance between the particle and the
     * ghost is < 2^(1/6)*((p.sigma + ghost.sigma)/2), instead of just 2^(1/6) * ghost.sigma(). This is because it could
     * happen that: 2^(1/6)*((p.sigma + ghost.sigma)/2) < 2^(1/6)*ghost.sigma. In other words this means if we used
     * sigma_ghost instead of sigma_check for checking the applicability of the boundary condition, if p.sigma >
     * ghost.sigma the ghost particle would actually attract the particle.
     *

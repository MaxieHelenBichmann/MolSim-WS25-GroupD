...
Reflecting<containerType>::Reflecting(containerType* particles, std::optional<double> counter_sigma,
                                      std::optional<double> counter_epsilon)
    : counter_epsilon(counter_epsilon), counter_sigma(counter_sigma) {
    this->particles = particles;
    /*
    if (!dimension.has_value()) {
        SPDLOG_ERROR(
            "Reflecting boundary condition specified but dimension for domain not provided! Defaulting dimension to "
            "{1.0, 1.0, 1.0}!");
    }*/
    max = R3{1., 1., 1.};
}
